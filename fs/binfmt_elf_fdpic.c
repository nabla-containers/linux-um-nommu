...
#define NEW_AUX_ENT(id, val)						\
	...
	kdebug("AT_BASE=%lx", interp_params->elfhdr_addr);
	...
	struct mm_struct *mm)
{
	...
	for (loop = 0; loop < params->hdr.e_phnum; loop++, phdr++) {
		...
		if (!load_addr_set) {
			load_addr_set = 1;
			params->load_addr = seg->addr - ELF_PAGESTART(phdr->p_vaddr);
			kdebug("load_addr %lx", params->load_addr);
		}

		kdebug("current=%lx read_code file=%lx addr=%lx off=%lx sz=%lx",
				current, file, seg->addr, phdr->p_offset,
				       phdr->p_filesz);
